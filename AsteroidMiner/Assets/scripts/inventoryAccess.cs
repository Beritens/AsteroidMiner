...
public class inventoryAccess : window
{
    ...
    void AddActive(Vector2Int item, int slot){
        activeManager.AddActive(item.x,slot,item.y);
    ...
    void uActive(){
        Vector2Int[] actives = inv.GetActives();
        for(int i = 0; i<actives.Length; i++){
            SetItem(actives[i],activeContainer.GetChild(i));
            AddActive(actives[i],i);
        }
    }
